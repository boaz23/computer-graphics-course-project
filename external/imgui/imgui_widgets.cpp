...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (p_step != NULL)
    {
        ...
        if (InputText("", buf, IM_ARRAYSIZE(buf), flags, callback, user_data)) // PushId(label) + "" gives us the expected ID from outside point of view
